...
class StorageUtils extends GetxService {
  ...
  Future<void> setToken(String token) async =>
      _setValue(AppConstants.tokenKey, token);
  Future<void> setUserName(String name) async =>
      _setValue(AppConstants.userName, name);
  Future<void> setUserId(String id) async =>
      _setValue(AppConstants.userIdKey, id);
  Future<void> setRoleId(String roleId) async =>
      _setValue(AppConstants.roleIdKey, roleId);
  Future<void> setIsLoggedIn(bool isLoggedIn) async =>
      _setValue(AppConstants.isLoggedInKey, isLoggedIn);
  Future<void> setItems(List<String> items) async =>
      _setValue(AppConstants.itemsKey, items);
  Future<void> setProfile(Map<String, dynamic> profile) async =>
      _setValue(AppConstants.profileKey, profile);
  Future<void> setLatitude(String value) async =>
      _setValue(AppConstants.latitudeKey, value);
  Future<void> setLongitude(String value) async =>
      _setValue(AppConstants.longitudeKey, value);
  Future<void> setFirstTimeLogin(String value) async =>
      _setValue(AppConstants.firstTimeLoginKey, value);
  Future<void> setDarkMode(String value) async =>
      _setValue(AppConstants.isDarkModeKey, value);
  Future<void> setLocal(String value) async =>
      _setValue(AppConstants.localeKey, value);
  Future<void> setLocationPermission(String value) async =>
      _setValue(AppConstants.locationPermission, value);
  Future<void> setNotificationPermission(String value) async =>
      _setValue(AppConstants.notificationPermission, value);
  Future<void> setLoggedInUserName(String name) async =>
      _setValue(AppConstants.loggedInUserName, name);
  Future<void> setLoggedInPassword(String name) async =>
      _setValue(AppConstants.loggedInPassword, name);
    ...
    if (isWeb) {
      ...
      for (final cookie in cookies) {
        ...
        html.document.cookie =
            '${cookiePair[0]}=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/';
      }
      ...
      if (Get.isRegistered<UserModel>()) {
        Get.delete<UserModel>(
          force: true,
        ); // Removes permanently registered instance
      }
