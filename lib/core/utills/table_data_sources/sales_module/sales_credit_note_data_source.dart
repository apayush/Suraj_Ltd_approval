...
class SalesCreditNoteDataSource extends DataGridSource {
  List<VoucherModel> salesCreditNoteList = [];
  List<VoucherModel> paginatedItems = [];
  int rowsPerPage;
  int currentPageIndex = 0;

  SalesCreditNoteDataSource(
    this.salesCreditNoteList, {
  ...
  void _loadInitialPage() {
    paginatedItems = salesCreditNoteList.take(rowsPerPage).toList();
    _buildDataGridRows();
  }
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    ...
    VoucherModel model = salesCreditNoteList[rowIndex];
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < salesCreditNoteList.length) {
      paginatedItems =
          salesCreditNoteList
              .getRange(
                startIndex,
                endIndex > salesCreditNoteList.length
                    ? salesCreditNoteList.length
                    : endIndex,
              )
              .toList();
  ...
  void updateDataSource(List<VoucherModel> updateList) {
    salesCreditNoteList = updateList;
    currentPageIndex = 0;
