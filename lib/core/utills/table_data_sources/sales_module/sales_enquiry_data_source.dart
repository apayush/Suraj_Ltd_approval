...
class SalesEnquiryDataSource extends DataGridSource {
  List<VoucherModel> salesEnquiryList = [];
  List<VoucherModel> paginatedItems = [];
  int rowsPerPage;
  int currentPageIndex = 0;

  SalesEnquiryDataSource(this.salesEnquiryList, {required this.rowsPerPage}) {
    _loadInitialPage();
  }
  ...
  void _loadInitialPage() {
    paginatedItems = salesEnquiryList.take(rowsPerPage).toList();
    _buildDataGridRows();
  }
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    ...
    VoucherModel model = salesEnquiryList[rowIndex];
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < salesEnquiryList.length) {
      paginatedItems =
          salesEnquiryList
              .getRange(
                startIndex,
                endIndex > salesEnquiryList.length
                    ? salesEnquiryList.length
                    : endIndex,
              )
              .toList();
