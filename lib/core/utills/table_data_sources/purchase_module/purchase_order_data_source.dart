...
class PurchaseOrderDataSource extends DataGridSource {
  List<VoucherModel> purchaseOrderList = [];
  List<VoucherModel> paginatedItems = [];
  int rowsPerPage;
  int currentPageIndex = 0;

  PurchaseOrderDataSource(this.purchaseOrderList, {required this.rowsPerPage}) {
    _loadInitialPage();
  }
  ...
  void _loadInitialPage() {
    paginatedItems = purchaseOrderList.take(rowsPerPage).toList();
    _buildDataGridRows();
  }
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    ...
    VoucherModel model = purchaseOrderList[rowIndex];
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < purchaseOrderList.length) {
      paginatedItems =
          purchaseOrderList
              .getRange(
                startIndex,
                endIndex > purchaseOrderList.length
