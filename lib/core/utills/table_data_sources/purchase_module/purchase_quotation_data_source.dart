...
class PurchaseQuotationDataSource extends DataGridSource {
  List<VoucherModel> purchaseQuotationList = [];
  List<VoucherModel> paginatedItems = [];
  int rowsPerPage;
  int currentPageIndex = 0;

  PurchaseQuotationDataSource(
      this.purchaseQuotationList, {
  ...
  void _loadInitialPage() {
    paginatedItems = purchaseQuotationList.take(rowsPerPage).toList();
    _buildDataGridRows();
  }
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    ...
    VoucherModel model = purchaseQuotationList[rowIndex];
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < purchaseQuotationList.length) {
      paginatedItems =
          purchaseQuotationList
              .getRange(
            startIndex,
            endIndex > purchaseQuotationList.length
                ? purchaseQuotationList.length
                : endIndex,
          )
              .toList();
      ...
      children: [
        Flexible(
          child: IconButton(
            ...
            tooltip: 'View',
            onPressed: () => controller.handleMenuSelection('View', model, fromPurchaseQuotation : true),
          ),
        ),
        Flexible(
          child: IconButton(
  ...
  void updateDataSource(List<VoucherModel> updateList) {
    purchaseQuotationList = updateList;
    currentPageIndex = 0;
