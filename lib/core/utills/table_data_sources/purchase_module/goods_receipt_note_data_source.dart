...
class GoodsReceiptNoteDataSource extends DataGridSource {
  List<VoucherModel> goodsReceiptNoteList = [];
  List<VoucherModel> paginatedItems = [];
  int rowsPerPage;
  int currentPageIndex = 0;

  GoodsReceiptNoteDataSource(this.goodsReceiptNoteList, {required this.rowsPerPage}) {
    _loadInitialPage();
  }
  ...
  void _loadInitialPage() {
    paginatedItems = goodsReceiptNoteList.take(rowsPerPage).toList();
    _buildDataGridRows();
  }
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    ...
    VoucherModel model = goodsReceiptNoteList[rowIndex];
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < goodsReceiptNoteList.length) {
      paginatedItems =
          goodsReceiptNoteList
              .getRange(
            startIndex,
            endIndex > goodsReceiptNoteList.length
                ? goodsReceiptNoteList.length
                : endIndex,
          )
              .toList();
  ...
  void updateDataSource(List<VoucherModel> updateList) {
    goodsReceiptNoteList = updateList;
    currentPageIndex = 0;
