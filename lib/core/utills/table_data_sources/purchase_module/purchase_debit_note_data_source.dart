...
class PurchaseDebitNoteDataSource extends DataGridSource {
  List<VoucherModel> purchaseDebitNoteList = [];
  List<VoucherModel> paginatedItems = [];
  int rowsPerPage;
  int currentPageIndex = 0;

  PurchaseDebitNoteDataSource(
      this.purchaseDebitNoteList, {
  ...
  void _loadInitialPage() {
    paginatedItems = purchaseDebitNoteList.take(rowsPerPage).toList();
    _buildDataGridRows();
  }
  ...
  DataGridRowAdapter buildRow(DataGridRow row) {
    ...
    VoucherModel model = purchaseDebitNoteList[rowIndex];
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    ...
    if (startIndex < purchaseDebitNoteList.length) {
      paginatedItems =
          purchaseDebitNoteList
              .getRange(
            startIndex,
            endIndex > purchaseDebitNoteList.length
                ? purchaseDebitNoteList.length
                : endIndex,
          )
              .toList();
  ...
  void updateDataSource(List<VoucherModel> updateList) {
    purchaseDebitNoteList = updateList;
    currentPageIndex = 0;
